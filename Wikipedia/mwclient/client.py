	...
	def site_init(self):
		...
		if self.site['generator'].startswith('MediaWiki '):
			...
			if len(version) == 2 and version[1].endswith('alpha'):
				...
				self.version = (int(version[0]), int(version[1][:-9]), 'alpha')
			elif len(version) == 3:
				self.version = (int(version[0]), int(version[1]), int(version[2]))
			#else:
			#  self.version = (1,7,'alpha') # Meh, it works
			#  print self.version
			# Commented out by theY4Kman (4/24/2011): stupid unknown version error for wikipedia.org
			#else:
